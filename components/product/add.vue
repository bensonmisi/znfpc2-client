...
export default {
 ...
 data(){
     return{
         ...
         nameRule:[v=>!!v || 'Service Product is required'],
         snackbar:false,
         color:'',
         text:'',
         loading:false
     ...
     async submit(){
       if(this.$refs.form.validate())
       {
          ...
          this.loading=true
             try {
                 await this.$axios.post('administrator/products',this.form).then((res)=>{
                        this.loading = false
                        this.color="success"
                        this.snackbar=true
                        this.text=res.data.message
                         this.$store.dispatch('product/getProducts',this.id)
