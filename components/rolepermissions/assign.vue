<template>
  <div>
      <v-btn x-small depressed color="success" @click="addPermModel=true">assign</v-btn>
      ...
      <v-dialog v-model="addPermModel" width="300">
       ...
       <v-card>
           ...
           <v-card-text class="text-center">
                ...
                <p>{{permission.name}} to {{ role.name}}</p>
           </v-card-text>
           <v-card-actions>
       ...
       if(this.$refs.form.validate())
       {
          ...
          this.loading=true
             try {
                 ...
                 await this.$axios.post('api/admin/role/assignpermission',data).then((res)=>{
                        this.loading = false
                        this.color="success"
                        this.snackbar=true
