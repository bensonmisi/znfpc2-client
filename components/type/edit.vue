...
export default {
 ...
 data(){
     return{
         ...
         nameRule:[v=>!!v || 'Service type name is required'],
         snackbar:false,
         color:'',
         text:'',
         loading:false
     ...
     async submit(){
       if(this.$refs.form.validate())
       {
          ...
          this.loading=true
             try {
                 await this.$axios.patch('administrator/types/'+this.type.id,this.form).then((res)=>{
                        this.loading = false
                        this.color="success"
                        this.snackbar=true
                        this.text=res.data.message
                        this.$store.dispatch('type/getTypes',this.type.id)
